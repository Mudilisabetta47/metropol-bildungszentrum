...
serve(async (req: Request) => {
  ...
  try {
    // --- Authentication: verify caller is a staff member ---
    ...
    // Verify staff role
    const { data: isStaffUser } = await supabaseAuth.rpc("is_staff", { _user_id: userId });
    if (!isStaffUser) {
      return new Response(
    ...
    // --- Proceed with invoice sending using service role key ---
